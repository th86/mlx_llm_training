...
- GRPO: Idea from [Doriandarko's MLX-GRPO](https://github.com/Doriandarko/MLX-GRPO). Fit the finetuning pipeline into M2 Pro 16GB using OpenCode by applying `gradient_accumulation_steps=1`. Run `uv run mlx-grpo.py --config configs/default.toml` in a `venv` environment.
